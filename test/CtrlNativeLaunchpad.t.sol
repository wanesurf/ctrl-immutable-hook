...
import {CtrlNativeTestBase} from "./CtrlNativeTestBase.sol";
...
contract CtrlNativeLaunchpadTest is CtrlNativeTestBase {
    using BalanceDeltaLibrary for BalanceDelta;
    using StateLibrary for IPoolManager;

    function testRejectsMismatchedHookEconomics() public {
        vm.expectRevert("hook tick mismatch");
        new CtrlNativeV4Factory(
            address(this),
            address(manager),
    ...
    function testRejectsInvalidNativeEconomicParameters() public {
        vm.expectRevert("zero launch fee");
        new CtrlNativeV4Factory(
            address(this),
            address(manager),
            address(positionManager),
        ...
        vm.expectRevert("zero graduation threshold");
        new CtrlNativeLaunchHook(
            address(manager), address(positionManager), address(vault), address(registry), address(this), 0, 204_200
        );
        vm.expectRevert("invalid initial tick");
        new CtrlNativeLaunchHook(
            address(manager),
            address(positionManager),
            address(vault),
    ...
    function testNativeHookHasNoUupsUpgradeEntryPoints() public {
        (bool hasUuid,) = address(hook).staticcall(abi.encodeWithSignature("proxiableUUID()"));
        (bool upgraded,) =
            address(hook).call(abi.encodeWithSignature("upgradeToAndCall(address,bytes)", USER, bytes("")));
        assertFalse(hasUuid);
        assertFalse(upgraded);
        assertEq(hook.GRADUATION_THRESHOLD(), 4.2 ether);
        assertEq(factory.initialSqrtPriceX96(), TickMath.getSqrtPriceAtTick(hook.INITIAL_TICK()));
    }

    function testFactoryDeploysPaused() public {
        CtrlNativeV4Factory freshFactory = new CtrlNativeV4Factory(
            address(this),
            address(manager),
            address(positionManager),
            address(permit2),
    ...
    function testLaunchCreatesFixedPoolAndPermanentlyLocksPosition() public {
        ...
        CtrlNativeV4Factory.LaunchRecord memory launched = factory.getLaunch(token);
    ...
    function testPredictAddressMatchesCreatorScopedCreate2Launch() public {
        CtrlNativeV4Factory.TokenParams memory params = _params();
    ...
    function testRevertingTreasuryRollsBackEntireLaunch() public {
        ...
        vm.expectRevert(CtrlNativeV4Factory.TreasuryTransferFailed.selector);
        ...
        vm.expectRevert(CtrlNativeLaunchHook.TokenNotRegistered.selector);
    ...
    function testExactInputBuyChargesOnePercentInEthAndSplitsFees() public {
        ...
        CtrlNativeLaunchHook.LaunchState memory launch = hook.getLaunch(token);
    ...
    function testPauseStopsOnlyFutureLaunches() public {
        ...
        vm.expectRevert(CtrlNativeV4Factory.LaunchesPaused.selector);
